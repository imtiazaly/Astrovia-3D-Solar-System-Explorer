...
export const TimeControls: React.FC<TimeControlsProps> = ({
  ...
  return (
    <div className="fixed bottom-3 sm:bottom-6 left-1/2 -translate-x-1/2 z-30 select-none max-w-[calc(100vw-16px)]">
      <div className="hud-glass px-3 sm:px-6 py-1.5 sm:py-2 rounded-full border border-cyan-500/35 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.8),0_0_25px_rgba(6,182,212,0.18)] flex items-center gap-2 sm:gap-4 text-cyan-200">
        ...
        <div className="relative group shrink-0">
          ...
          >
            {timeEngine.isPlaying ? (
              ...
              <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-cyan-300 ml-0.5" />
            )}
          </button>
        ...
        <div className="relative group shrink-0">
          <button
            ...
            className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl text-xs font-mono-hud font-bold transition-all border cursor-pointer active:scale-95 flex items-center gap-1 sm:gap-1.5 ${
          ...
          >
            <Compass
              className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${timeEngine.isReversed ? "text-amber-400 rotate-180" : "text-cyan-400"}`}
            />
            <span className="text-[9px] sm:text-[10px] tracking-wider uppercase hidden xs:inline">
              {timeEngine.isReversed ? "Retrograde" : "Prograde"}
            </span>
            <span className="text-[9px] tracking-wider uppercase xs:hidden">
              {timeEngine.isReversed ? "REV" : "FWD"}
            </span>
          </button>
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-2.5 py-1 bg-slate-900/95 border border-cyan-500/40 rounded-xl shadow-xl text-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap hidden sm:block">
            <div className="text-[10px] font-bold text-cyan-300 font-mono-hud">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 bg-slate-950/90 p-0.5 sm:p-1 rounded-2xl border border-cyan-500/25">
          <Gauge className="w-3.5 h-3.5 text-cyan-400 ml-1.5 mr-0.5 hidden md:block" />
          {speeds.map((s) => (
            <button
              key={s}
              onClick={() => {
        ...
        <div className="relative group shrink-0">
          <button
            ...
            className="p-1.5 sm:p-2 rounded-xl text-slate-400 hover:text-cyan-300 hover:bg-slate-900/80 transition-all cursor-pointer active:scale-95"
            title="Reset Time (1x)"
          >
